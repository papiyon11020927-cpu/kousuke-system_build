 ...
 *
 * ── onCall を v1 API で実装している理由 ──────────────────────────
 *   v2 (Cloud Run) の onCall は Cloud Run IAM が OPTIONS プリフライトを
 *   ブロックするため CORS エラーが発生する。v1 は Firebase インフラが
 *   CORS と認証を透過的に処理するため IAM 設定が不要。
 */
...
// ════════════════════════════════════════════════════════════════════
export const analyzeVendorDoc = functions
  .region('asia-northeast1')
  .runWith({ secrets: [geminiSecret], maxInstances: 5, timeoutSeconds: 60 })
  .https.onCall(async (data: { base64?: string; mimeType?: string }) => {
...
// ════════════════════════════════════════════════════════════════════
export const geminiAnalyzeReport = functions
  .region('asia-northeast1')
  .runWith({ secrets: [geminiSecret], maxInstances: 10, timeoutSeconds: 30 })
  .https.onCall(async (data: { text?: string }, context) => {
    if (!context.auth) throw new HttpsError('unauthenticated', '認証が必要です');
    const { text } = data;
...
// ════════════════════════════════════════════════════════════════════
export const geminiDailySummary = functions
  .region('asia-northeast1')
  .runWith({ secrets: [geminiSecret], maxInstances: 10, timeoutSeconds: 30 })
  .https.onCall(async (data: { staffName?: string; logText?: string }, context) => {
    if (!context.auth) throw new HttpsError('unauthenticated', '認証が必要です');
    const { staffName, logText } = data;
...
export const signCustomerContract = functions
  .region('asia-northeast1')
  .runWith({ maxInstances: 10, timeoutSeconds: 30 })
  .https.onCall(async (data: { contractId?: string; signatureDataUrl?: string }) => {
